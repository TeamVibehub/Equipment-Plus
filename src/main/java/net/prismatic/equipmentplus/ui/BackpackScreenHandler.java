...
public class BackpackScreenHandler extends ScreenHandler
{
    ...
    public BackpackScreenHandler(final int syncId, final PlayerInventory playerInventory, final Inventory inventory, final int inventoryWidth, final int inventoryHeight)
    {
        super(null, syncId);
    ...
    public ItemStack onSlotClick(int slotId, int clickData, SlotActionType actionType, PlayerEntity playerEntity)
    {
        if (slotId >= 0) // slotId < 0 are used for networking internals, to avoid array out of range (-999)
        {
            ...
            if((stack.getItem() instanceof Backpack)  ||  (actionType == SlotActionType.SWAP))
            {
    ...
    public ItemStack transferSlot(final PlayerEntity player, final int invSlot)
    {
        ...
        if (!(testItem instanceof Backpack))
        {
